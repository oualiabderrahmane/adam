...
public class Chambre {
    ...
    private void verifierContraintesStructure() {
        if (type == TypeChambre.DOUBLE && lits.size() != 2) {
            throw new IllegalArgumentException(
                "Une chambre double doit contenir exactement 2 lits separes."
            );
        }

        if (type == TypeChambre.STANDARD && lits.size() != 4) {
            throw new IllegalArgumentException(
                "Une chambre standard doit contenir exactement 4 lits."
            );
        }
    }
    ...
    public boolean ajouterOccupant(Personne personne) {
        ...
        if (contient(personne)) {
            System.out.println(
                "Erreur : " + personne.getNom() + " a deja reserve la chambre " + code + "."
            );
            return false;
        }
